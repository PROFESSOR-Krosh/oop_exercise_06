...
int main() {
    ...
    int index, val;
    Stack<Rectangle<int>,my_allocator<Rectangle<int>,330>> t;
    ...
    while(option != "Ex"){
        if (option == "Add") {
            ...
            std::for_each(t.begin(), t.end(), [&num](auto e) {
                ...
                std::cout << "Rectangle area " << e.area_ << std::endl;
                num++;
            });
        } else if(option == "Top") {
            try {
            ...
            std::cin >> val;
            std::cout << "The number of figures with an area less than a given "<< std::count_if(t.begin(),t.end(), [val](Rectangle<int> Rec)
            {return Rec.area_ < val;})  << std::endl;
        } else if (option == "Menu") {
            std::cout << "Enter a word for action:" << std::endl;
            std::cout << "1) Add Rectangle to the stack[Add]" <<
